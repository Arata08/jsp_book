...
<div align="center"><h1 style="color:steelblue;font-family:YouYuan;font-weight:900;">读者规则信息</h1></div>
<div style="margin-top:0%;" align="center">
<%		
		String sql = "select * from rules";
		ResultSet rs = msg.executeQuery(sql);
		while (rs.next()) {
		if(Integer.parseInt(rs.getString("ID")) % 2== 1){//奇偶样式不同样
%>
	<button type="button" class="btn btn-lg btn-info" title="可借阅数量：<%=rs.getString("BORROW_NUM") %>" data-html="true"
			...
			data-content="可借阅天数：<%=rs.getString("LIMIT_DAY") %><br>可借阅图书馆：<%=rs.getString("BORROW_LIBRARY") %>
			<br>过期扣费/天：<%=rs.getString("OVERTIME_FEE") %>" style="width:40%;height:100%;font-size:17px;margin-top:4%;">
